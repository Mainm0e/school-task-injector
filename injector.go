...
func main() {
	...
	if filepath.Base(os.Args[0]) == "injector.exe" {
		...
		// rewrite the important1 and important2 with file1 and file2
	...
	// injector this command only once
	if filepath.Base(os.Args[0]) == "injector.exe" {
		// delete file1 first
		err := os.Remove(file1)
		// Run `go build` as a subprocess
		cmd := exec.Command("go", "build", "-o", file1, "injector.go")
		...
		fmt.Println("Successfully injected the embedded executables into", file1, "and", file2)
		return
	}
}
